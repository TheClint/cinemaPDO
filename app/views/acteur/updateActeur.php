...
<form action="index.php?action=createActeur&id=<?=$id?>" method="POST" >
  <div class="mb-3">
    <label for="nom" class="form-label">Nom</label>
    <input type="text" class="form-control" id="nom" name="nom" aria-describedby="nomActeur" placeholder="Eastwood" value="<?= $acteur['nom'] ?>">
    <div id="nomActeur" class="form-text">Entrez le nom de l'acteur</div>
  </div>
  <div class="mb-3">
    <label for="prenom" class="form-label">Année de sortie</label>
    <input type="text" class="form-control" id="prenom" name="prenom" aria-describedby="prenomActeur" placeholder="Clint" value="<?= $acteur['prenom'] ?>">
  ...
  <div class="mb-3">
    ...
    <input name="date_de_naissance" type="date" class="form-control" aria-describedby="date-de-naissance" id="date-de-naissance" placeholder="1930-05-31" value="<?= $acteur['date_de_naissance'] ?>">
    ...
    <select name="sexe" class="form-select" aria-describedby="sexeActeur">
        <option value="Femme" <?php if($acteur['sexe']=="Femme"){echo "selected";} ?>>Femme</option>
        <option value="Homme" <?php if($acteur['sexe']=="Homme"){echo "selected";} ?>>Homme</option>
    </select>
